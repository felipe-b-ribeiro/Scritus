...
import {
  ...
  validarEmail,
  validarSenhaForte,
} from "../../common/util/validations.js";
import { CAMPOS } from "../../frontend/src/constants/userConstants.js";
import {
  criarUsuarioRepository,
  encontrarUsuarioPorInfoRepository,
  pullDataUserRepository,
...
export const criarUsuarioService = async (dadosFront) => {
  try {
    ...
    Object.values(dadosUsuario).forEach((campo) => {
      if (!campo || !Object.hasOwn(campo, "valor")) {
    ...
    if (
      (tipo.valor === "leitor" || tipo.valor === "autor") &&
    ...
    switch (tipo) {
      ...
      case "Autor":
        ...
        if (!dataNascimento.valor)
          throw new Error("A data de nascimento é obrigatória");
        if (pseudonimo.valor && pseudonimo.valor.length < 3)
          throw new Error("O pseudônimo deve conter no mínimo 3 caracteres!");
        break;
      case "Editora":
        if (!nomeFantasia.valor)
        ...
        if (!(siteOficial.valor.includes("."))) throw new Error("O site oficial é inválido!");
        break;
      default:
        throw new Error("Tipo de usuário inválido!");
    }

    if (!validarSenhaForte(senha.valor))
      throw new Error("A senha digitada não é forte.");
    if (senha.valor.length > 30)
      throw new Error("A senha deve conter no máximo 30 caracteres!");
    ...
    let dadosCriptografados = {
      ...
      dados: Object.fromEntries(
        Object.entries(dadosUsuario)
          ...
          .map(([key, campos]) => [
            key,
            key === "senha" ? senhaCriptografada : campos.valor,
          ]),
      ),
    };
    ...
    if (cnpj) {
      ...
      dadosCriptografados = {
        ...dadosCriptografados,
        dados: { ...dadosCriptografados.dados, cnpj: cnpjSemMascara },
      };
    }
    ...
    const payload = await criarUsuarioRepository(dadosPraEnviar);
...
export const pullDataUserService = async (info) => {
  try {
    if (!info.tipoUsuario || !info.email)
      throw new Error("Campos inválidos para pegar os dados.");
