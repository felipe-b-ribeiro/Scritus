import {
  ...
  puxarPerfilRepository,
} from "../repositories/userRepository.js";
import {
  criarUsuarioService,
  encontrarUsuarioPorInfoService,
  pullDataUserService,
  updateUserService,
...
export const createUser = async (req, res) => {
  try {
    const token = await criarUsuarioService(req.body);
    if (!token) return res.status(401).json({ erro: "Falha ao criar conta!" });
...
export const findUserbyInfo = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      campo: Object.keys(info)[0],
      valor: Object.values(info)[0],
      disponivel: resposta,
    });
  } catch (err) {
    console.error("[USER CONTROLLER ERROR]:", err);
...
export const updateUser = async (req, res) => {
  try {
    ...
    const foto_perfil = req.file
      ? `/uploads/fotos_perfis/${req.file.filename}`
      : null;
