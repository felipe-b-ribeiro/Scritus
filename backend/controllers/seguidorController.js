import { criarSeguidorRepository, deletarSeguidorRepository, verificarSeSegueRepository } from '../repositories/seguidorRepository.js';

export const criarSeguidorController = async (req, res) => {
    try {
        const {id_perfil, id_perfil_seguidor} = req.body;
        const seguidor = await criarSeguidorRepository(id_perfil_seguidor, id_perfil);
...
export const deletarSeguidorController = async (req, res) => {
    try {
        const {id_perfil, id_perfil_seguidor} = req.body;
        const seguidor = await deletarSeguidorRepository(id_perfil_seguidor, id_perfil);
        res.status(200).json(seguidor);
    } catch (err) {
