...
function PaginaRecentes() {
    ...
    useEffect(() => {
        const carregarPagina = async () => {
            try {
                ...
                const resposta = await fetch(`http://localhost:5000/api/v1/obra/salvos?idPerfil=${id_perfil}&tipoInteracao=clique`)
    ...
    return (
        ...
        <ContainerBasico width='90vw'>
            <SC_TagGrande cor='purple'>
                Seus <strong>Recentes</strong>
            </SC_TagGrande>
            <SC_Info cor='purple'>
                <strong>{obras.length}</strong>{ obras.length === 1 ? ' livro acessado recentemente' : ' livros acessados recentemente'}
            </SC_Info>
            {obras.length === 0 ? (
