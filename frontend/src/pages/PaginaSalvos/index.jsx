...
function PaginaSalvos() {
    ...
    useEffect(() => {
        const carregarPagina = async () => {
            try {
                ...
                const resposta = await fetch(`http://localhost:5000/api/v1/obra/salvos?idPerfil=${id_perfil}&tipoInteracao=salvo`)
    ...
    return (
        ...
        <ContainerBasico width='90vw'>
            <SC_TagGrande cor='blue'>
                Seus <strong>Salvos</strong>
            </SC_TagGrande>
            <SC_Info cor='blue'>
                <strong>{obras.length}</strong>{ obras.length === 1 ? ' livro salvo' : ' livros salvos'}
            </SC_Info>
            {obras.length === 0 ? (
